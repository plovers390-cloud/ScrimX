...
class ScrimsSlotManagerSetup(EsportsBaseView):
    ...
    async def initial_message(guild: discord.Guild):
        ...
        _e.description = (
            "Slot-Manager is a way to ease-up scrims slot management process. With ScrimX's slotm users can - "
    ...
    async def add_channel(self, interaction: discord.Interaction, button: discord.Button):
        ...
        if not await self.ctx.is_premium_guild():
            if await ScrimsSlotManager.filter(guild_id=self.ctx.guild.id).count() >= 1:
                return await self.ctx.premium_mango("You need ScrimX Premium to add more than 1 Slot-Manager channel.")

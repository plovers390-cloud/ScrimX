...
class TourneyEvents(Cog):
            ...
            if role := tourney.modrole:
                if role in after.roles and not role in before.roles:
                    msg = (
                        ...
                        "• You can now use all `xtourney` commands.\n"
    ...
    async def on_guild_channel_update(self, before: discord.TextChannel, after: discord.TextChannel):
        if before.name == after.name or not before.name == "ScrimX-tourney-logs":
            return
        ...
        _e = discord.Embed(
            color=discord.Color.red(),
            description=(
                "Someone renamed this channel kindly rename it back to `ScrimX-tourney-logs`, "
                "**ScrimX Tourneys won't work without it.**"
            ),
        )
        await after.send(
    ...
    async def on_guild_role_update(self, before: discord.Role, after: discord.Role):
        ...
        _e = discord.Embed(
            color=discord.Color.red(),
            description=(
                f"Someone renamed ScrimX's tourney-mod role to {after.mention}, kindly rename it back to `tourney-mod`."
                "**ScrimX Tourneys need the name of this role to be `tourney-mod`**"
            ),
        )

        c = discord.utils.get(after.guild.text_channels, name="ScrimX-tourney-logs")
        if c:
            await c.send(
                embed=_e, content=getattr(after.guild.owner, "mention")
            )  # there is very less chances of getting attribute error on `owner.mention`
